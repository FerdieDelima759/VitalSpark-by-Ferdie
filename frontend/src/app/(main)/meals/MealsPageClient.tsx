...
export default function MealsPageClient({
  ...
  dayGenerationConcurrency = 1,
  showBackToMealsButton = false,
  showRefreshButton = false,
}: MealsPageClientProps) {
  const router = useRouter();
  const workoutPlanId = workoutPlanIdOverride ?? null;
  ...
  const handleGenerateOverview = useCallback(
        ...
        if (result.success && result.response) {
          ...
          if (dayEntries.length > 0) {
            ...
            const concurrency = Math.max(
              1,
              Math.min(dayGenerationConcurrency, dayEntries.length),
            );
            let cursor = 0;
    ...
    [activeDay, dayGenerationConcurrency, getMealPlanVariables, userProfile],
  );
      ...
      return (
        <div className="rounded-xl border border-slate-200 dark:border-teal-600 bg-white dark:bg-teal-500/80 px-3 py-4 shadow-sm">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">{mealName}</p>
          </div>
          {hasInstructions && (
            <button
              type="button"
          ...
          <div className="text-sm text-slate-700 dark:text-slate-300">
            <p className="font-semibold text-slate-600 dark:text-slate-400 mb-2">Ingredients</p>
  ...
  const showWorkoutContext = !!workoutPlanId && !checkingLink;
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-extrabold text-slate-900 dark:text-slate-100">Meals</h1>
              <p className="text-sm text-slate-500 dark:text-slate-400">
            ...
            >
              ...
              {linkedPlansForWorkout.length > 0 ? (
                <div className="space-y-3">
                  {linkedPlansForWorkout.map((linkedPlan) => (
                    <button
                      key={linkedPlan.id}
                      type="button"
                      onClick={() =>
                  ...
                  <button
                    ...
                    className="w-full py-2.5 rounded-xl bg-teal-600 dark:bg-teal-500 text-white text-sm font-semibold hover:bg-teal-700 dark:hover:bg-teal-400 disabled:opacity-60 transition-colors"
                  >
                    {isLoadingProfile
                      ? "Loading profile..."
                      : !userProfile
                ...
                <>
                  <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                    No meal plan linked to this workout yet. Attach one from
                    your workout details.
                  </p>
                  <div className="rounded-xl bg-gradient-to-br from-teal-50 to-emerald-50 dark:from-teal-900/30 dark:to-emerald-900/30 border border-teal-100 dark:border-teal-700 p-4">
                    <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 mb-1">
                      Personalized Meal Plan
                    </h3>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mb-4">
                      Get a meal plan tailored to your workout and goals.
                    </p>
                    <button
                      type="button"
                      disabled={
                      ...
                      onClick={openAllergyDialog}
                      className="w-full py-3 rounded-xl bg-teal-600 dark:bg-teal-500 text-white font-semibold hover:bg-teal-700 dark:hover:bg-teal-400 disabled:opacity-60 transition-colors"
                    >
                      {generateCtaLabel}
                    </button>
                    {!userProfile && !isLoadingProfile && (
                      <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
                        Open Personal to complete your profile (diet, budget,
                        etc.).
                      </p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {userWorkoutPlans.map((plan) => (
                    <button
                      key={plan.id}
                      type="button"
                      onClick={() => router.push(`/meals/workout/plan/${plan.id}`)}
