...
import { WorkoutPlan, WorkoutTag } from "@/types/Workout";
...
interface WorkoutPlanWithTags extends WorkoutPlan {
  tags?: WorkoutTag[];
}
...
export default function GeneralWorkoutsPage() {
  ...
  const [workoutPlans, setWorkoutPlans] = useState<WorkoutPlanWithTags[]>([]);
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        showToast(
          "error",
          "Error",
          err.message || "Failed to fetch workout plans"
        );
      } finally {
        setIsLoadingPlans(false);
      }
  ...
  const handleEditClick = (plan: WorkoutPlanWithTags) => {
    setFormData({
      name: plan.name,
  ...
  const handleDeleteClick = (plan: WorkoutPlan) => {
    setShowDeleteDialog({
      visible: true,
      planId: plan.id,
      planName: plan.name,
  ...
  const handleDeleteSelectedClick = () => {
    ...
    const selectedNames = workoutPlans
  ...
  const toggleSelectAll = () => {
    if (selectedPlans.length === workoutPlans.length) {
      setSelectedPlans([]);
    } else {
      setSelectedPlans(workoutPlans.map((p) => p.id));
    }
  };
    ...
    try {
      ...
      if (editingPlan) {
        ...
        if (insertError) {
          ...
          console.error(
            "Insert error properties:",
            Object.getOwnPropertyNames(insertError || {})
          );
          console.error(
            "Insert error stringified:",
      ...
      // Use requestAnimationFrame to ensure DOM has updated, then add delay
      ...
      // Check common Supabase error properties (PostgrestError structure)
      if (err?.message) {
        ...
        try {
          const errorStr = JSON.stringify(
            ...
            (key, value) => {
              ...
              if (key === "parent" || key === "originalError")
                return "[Circular]";
              return value;
            },
            2
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      showToast(
        "success",
        "Success",
        `${count} workout plan${count > 1 ? "s" : ""} deleted successfully`
      );
      ...
      const errorMessage = formatConstraintError(err, "workout plan");
      showToast("error", "Error", errorMessage);
    } finally {
      setIsDeleting(false);
    }
  ...
  const formatConstraintError = (err: any, entityName: string): string => {
    ...
    if (
      ...
      // Common patterns: "violates foreign key constraint", "still referenced from table", etc.
      ...
      if (tableMatch && tableMatch[1]) {
        ...
        if (
          ...
          referencedEntity = "workout plan exercises";
                      ...
                      <th className="px-6 py-3 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider w-12">
                        <input
                          ...
                          checked={
                            ...
                            selectedPlans.length === workoutPlans.length
                          }
                        ...
                        <tr key={plan.id} className="hover:bg-slate-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <button
                              ...
                              onClick={() =>
                                router.push(
                                  `/admin/general/workouts/plan/${plan.id}`
                                )
                              }
                              className="px-3 py-1.5 text-xs font-medium bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-sm"
                            >
                              Add Exercises
                            </button>
                          </td>
          ...
          <p className="text-slate-700 mb-6">
            {showDeleteDialog.planIds && showDeleteDialog.planIds.length > 1 ? (
              <>
                ...
                {showDeleteDialog.planIds.length} workout plans?
                <br />
