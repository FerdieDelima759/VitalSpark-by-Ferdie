...
if (!OPENAI_API_KEY) {
    console.warn(
        "Missing NEXT_PUBLIC_OPENAI_API_KEY. Set it in your .env.local file to use OpenAI services."
    );
}
...
export const openai = OPENAI_API_KEY ? new OpenAI({
    ...
    dangerouslyAllowBrowser: true, // Required for browser compatibility
}) : null;

// Check if OpenAI service is available and working
export async function isOpenAIAvailable(): Promise<boolean> {
    if (!openai) {
        console.warn("OpenAI client is not initialized. Please set NEXT_PUBLIC_OPENAI_API_KEY in .env.local");
        return false;
    }
    try {
