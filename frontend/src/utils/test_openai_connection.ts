...
export async function testOpenAIConnectionFull(): Promise<boolean> {
    try {
        ...
        if (!openai) {
            ...
            console.error('   Please set NEXT_PUBLIC_OPENAI_API_KEY in your .env.local file');
            return false;
        }
        ...
        if (!apiKey || apiKey.trim() === '') {
            console.error('❌ NEXT_PUBLIC_OPENAI_API_KEY not configured');
            console.error('   Please add NEXT_PUBLIC_OPENAI_API_KEY=your-api-key to .env.local');
            return false;
        }

        if (apiKey.includes('your-api-key') || apiKey.includes('sk-') === false) {
            console.warn('⚠️  NEXT_PUBLIC_OPENAI_API_KEY may not be configured correctly');
